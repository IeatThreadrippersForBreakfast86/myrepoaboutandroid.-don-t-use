...
public class WKSRecord extends Record {
    ...
    public static class Protocol {
        ...
        /* renamed from: ST */
        public static final int f282ST = 5;
        ...
        private static final Mnemonic protocols = new Mnemonic("IP protocol", 3);
    ...
    public static class Service {
        ...
        private static final Mnemonic services = new Mnemonic("TCP/UDP service", 3);
    ...
    public WKSRecord(Name name, int dclass, long ttl, InetAddress address, int protocol, int[] services) {
        ...
        for (int service : services) {
            checkU16(NotificationCompat.CATEGORY_SERVICE, service);
        }
    ...
    protected void rrFromWire(DNSInput in) throws IOException {
        ...
        for (int i = 0; i < array.length; i++) {
            for (int j = 0; j < 8; j++) {
                int octet = array[i] & UByte.MAX_VALUE;
                if (((1 << (7 - j)) & octet) != 0) {
        ...
        int i2 = list.size();
        this.services = new int[i2];
        for (int i3 = 0; i3 < list.size(); i3++) {
            this.services[i3] = list.get(i3).intValue();
        }
    }
        ...
        while (true) {
            ...
            if (t.isString()) {
                ...
                for (int i = 0; i < list.size(); i++) {
                    this.services[i] = list.get(i).intValue();
                }
                return;
            }
        }
    ...
    protected void rrToWire(DNSOutput out, Compression c, boolean canonical) {
        ...
        for (int port : this.services) {
            int i = port / 8;
            array[i] = (byte) (array[i] | ((byte) (1 << (7 - (port % 8)))));
        }
