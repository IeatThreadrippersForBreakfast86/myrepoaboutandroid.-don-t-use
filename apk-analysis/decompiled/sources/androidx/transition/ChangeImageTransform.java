...
public class ChangeImageTransform extends Transition {
    ...
    private void captureValues(TransitionValues transitionValues, boolean useIntermediate) {
        ...
        Matrix matrix = null;
        if (useIntermediate) {
            matrix = (Matrix) imageView.getTag(C0571R.id.transition_image_transform);
        }
        if (matrix == null) {
    ...
    private static Matrix copyImageMatrix(ImageView view) {
        ...
        if (image.getIntrinsicWidth() > 0 && image.getIntrinsicHeight() > 0) {
            switch (C05613.$SwitchMap$android$widget$ImageView$ScaleType[view.getScaleType().ordinal()]) {
                case 1:
    ...
    private static class Listener extends AnimatorListenerAdapter implements Transition.TransitionListener {
        ...
        private void restoreMatrix() {
            Matrix pauseMatrix = (Matrix) this.mImageView.getTag(C0571R.id.transition_image_transform);
            if (pauseMatrix != null) {
                ImageViewUtils.animateTransform(this.mImageView, pauseMatrix);
                this.mImageView.setTag(C0571R.id.transition_image_transform, null);
            }
        }

        private void saveMatrix(Matrix pauseMatrix) {
            this.mImageView.setTag(C0571R.id.transition_image_transform, pauseMatrix);
