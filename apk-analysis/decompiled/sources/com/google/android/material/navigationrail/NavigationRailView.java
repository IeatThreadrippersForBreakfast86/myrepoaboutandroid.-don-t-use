...
public class NavigationRailView extends NavigationBarView {
    ...
    public NavigationRailView(Context context, AttributeSet attrs) {
        this(context, attrs, C0633R.attr.navigationRailStyle);
    }

    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, C0633R.style.Widget_MaterialComponents_NavigationRailView);
    }

    public NavigationRailView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) throws Resources.NotFoundException {
        ...
        this.topMargin = res.getDimensionPixelSize(C0633R.dimen.mtrl_navigation_rail_margin);
        Context context2 = getContext();
        TintTypedArray attributes = ThemeEnforcement.obtainTintedStyledAttributes(context2, attrs, C0633R.styleable.NavigationRailView, defStyleAttr, defStyleRes, new int[0]);
        int headerLayoutRes = attributes.getResourceId(C0633R.styleable.NavigationRailView_headerLayout, 0);
        if (headerLayoutRes != 0) {
            addHeaderView(headerLayoutRes);
        }
        setMenuGravity(attributes.getInt(C0633R.styleable.NavigationRailView_menuGravity, 49));
        if (attributes.hasValue(C0633R.styleable.NavigationRailView_itemMinHeight)) {
            setItemMinimumHeight(attributes.getDimensionPixelSize(C0633R.styleable.NavigationRailView_itemMinHeight, -1));
        }
        if (attributes.hasValue(C0633R.styleable.NavigationRailView_paddingTopSystemWindowInsets)) {
            this.paddingTopSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0633R.styleable.NavigationRailView_paddingTopSystemWindowInsets, false));
        }
        if (attributes.hasValue(C0633R.styleable.NavigationRailView_paddingBottomSystemWindowInsets)) {
            this.paddingBottomSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0633R.styleable.NavigationRailView_paddingBottomSystemWindowInsets, false));
        }
        if (attributes.hasValue(C0633R.styleable.NavigationRailView_paddingStartSystemWindowInsets)) {
            this.paddingStartSystemWindowInsets = Boolean.valueOf(attributes.getBoolean(C0633R.styleable.NavigationRailView_paddingStartSystemWindowInsets, false));
        }
        int largeFontTopPadding = getResources().getDimensionPixelOffset(C0633R.dimen.m3_navigation_rail_item_padding_top_with_large_font);
        int largeFontBottomPadding = getResources().getDimensionPixelOffset(C0633R.dimen.m3_navigation_rail_item_padding_bottom_with_large_font);
