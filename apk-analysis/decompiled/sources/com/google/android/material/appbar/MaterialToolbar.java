...
public class MaterialToolbar extends Toolbar {
    private static final int DEF_STYLE_RES = C0633R.style.Widget_MaterialComponents_Toolbar;
    ...
    public MaterialToolbar(Context context, AttributeSet attrs) {
        this(context, attrs, C0633R.attr.toolbarStyle);
    }

    public MaterialToolbar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(MaterialThemeOverlay.wrap(context, attrs, defStyleAttr, DEF_STYLE_RES), attrs, defStyleAttr);
        Context context2 = getContext();
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(context2, attrs, C0633R.styleable.MaterialToolbar, defStyleAttr, DEF_STYLE_RES, new int[0]);
        if (a.hasValue(C0633R.styleable.MaterialToolbar_navigationIconTint)) {
            setNavigationIconTint(a.getColor(C0633R.styleable.MaterialToolbar_navigationIconTint, -1));
        }
        this.titleCentered = a.getBoolean(C0633R.styleable.MaterialToolbar_titleCentered, false);
        this.subtitleCentered = a.getBoolean(C0633R.styleable.MaterialToolbar_subtitleCentered, false);
        int index = a.getInt(C0633R.styleable.MaterialToolbar_logoScaleType, -1);
        if (index >= 0 && index < LOGO_SCALE_TYPE_ARRAY.length) {
            this.logoScaleType = LOGO_SCALE_TYPE_ARRAY[index];
        }
        if (a.hasValue(C0633R.styleable.MaterialToolbar_logoAdjustViewBounds)) {
            this.logoAdjustViewBounds = Boolean.valueOf(a.getBoolean(C0633R.styleable.MaterialToolbar_logoAdjustViewBounds, false));
        }
        a.recycle();
