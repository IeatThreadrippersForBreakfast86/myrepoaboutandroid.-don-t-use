...
public final class DispatchedContinuation<T> extends DispatchedTask<T> implements CoroutineStackFrame, Continuation<T> {
    ...
    public final CancellableContinuationImpl<T> claimReusableCancellableContinuation$kotlinx_coroutines_core() {
        ...
        while (true) {
            ...
            if (state instanceof CancellableContinuationImpl) {
                if (C0143xc40028dd.m5m(_reusableCancellableContinuation$volatile$FU, this, state, DispatchedContinuationKt.REUSABLE_CLAIMED)) {
    ...
    public final Throwable tryReleaseClaimedContinuation$kotlinx_coroutines_core(CancellableContinuation<?> continuation) {
        ...
        do {
            ...
            if (state != DispatchedContinuationKt.REUSABLE_CLAIMED) {
                if (state instanceof Throwable) {
                    if (!C0143xc40028dd.m5m(_reusableCancellableContinuation$volatile$FU, this, state, null)) {
        ...
        } while (!C0143xc40028dd.m5m(_reusableCancellableContinuation$volatile$FU, this, DispatchedContinuationKt.REUSABLE_CLAIMED, continuation));
        return null;
    }
    ...
    public final boolean postponeCancellation$kotlinx_coroutines_core(Throwable cause) {
        ...
        while (true) {
            ...
            if (Intrinsics.areEqual(state, DispatchedContinuationKt.REUSABLE_CLAIMED)) {
                if (C0143xc40028dd.m5m(_reusableCancellableContinuation$volatile$FU, this, DispatchedContinuationKt.REUSABLE_CLAIMED, cause)) {
                    return true;
                }
            } else {
                if (state instanceof Throwable) {
    ...
    public void resumeWith(Object result) {
        ...
        if (this.dispatcher.isDispatchNeeded(context)) {
            this._state = state;
            this.resumeMode = 0;
            this.dispatcher.mo1813dispatch(context, this);
            return;
        }
        if (DebugKt.getASSERTIONS_ENABLED()) {
        }
    ...
    */
    public final void resumeCancellableWith$kotlinx_coroutines_core(Object result, Function1<? super Throwable, Unit> onCancellation) {
        ...
        if (this.dispatcher.isDispatchNeeded(getContext())) {
            this._state = state;
            this.resumeMode = 1;
            this.dispatcher.mo1813dispatch(getContext(), this);
            return;
        }
        if (DebugKt.getASSERTIONS_ENABLED()) {
        }
            ...
            if (job$iv != null) {
                try {
                    if (!job$iv.isActive()) {
                        ...
                        resumeWith(Result.m255constructorimpl(ResultKt.createFailure(cause$iv)));
                        z = true;
                    } else {
                        z = false;
                    }
